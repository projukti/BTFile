...
if(isset($_REQUEST['mode']) && $_REQUEST['mode']=='fileadd'){
	...
	$add_file= mysql_query("insert into `files` set
								...
								`drive`='".	strtoupper($_REQUEST['drive'])."'");
...
<head>
    ...
    <style>
		...
		.panel2 {display: block; background: rgba(255,255,255,0.7); width: 800px; padding: 15px; margin: 30px auto; border: 1px solid rgb(13,90,190)}
        form {display: table}
...
if(isset($_REQUEST['deleted'])){echo '<span class="err">Deleted Successfully</span>';}?>
    <form action="" name="filem" id="filem" method="post" enctype="application/x-www-form-urlencoded">
        ...
        <label for="tags">Directory Name<br></label>
        <input name="drive" id="drive" placeholder="C" value="<?php if(isset($_REQUEST['id'])){ echo $sel_query['drive'];}?>">
...
<div class="panel2">
    <form action="" name="filem" id="filem" method="post" enctype="application/x-www-form-urlencoded">
    	<table width="800" border="0" cellspacing="0" cellpadding="0" style="width:800px">
  <tr>
    <td width="35%">File</td>
    <td width="35%">Tags</td>
    <td width="15%">Edit</td>
